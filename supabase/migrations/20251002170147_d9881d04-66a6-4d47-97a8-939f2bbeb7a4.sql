...
DROP POLICY IF EXISTS "Users can view their own gamification profile" ON user_gamification_profiles;
CREATE POLICY "Users can view their own gamification profile" ON user_gamification_profiles
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own gamification profile" ON user_gamification_profiles;
CREATE POLICY "Users can update their own gamification profile" ON user_gamification_profiles
  FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own gamification profile" ON user_gamification_profiles;
CREATE POLICY "Users can insert their own gamification profile" ON user_gamification_profiles
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can view their own points log" ON user_points_log;
CREATE POLICY "Users can view their own points log" ON user_points_log
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "System can insert points for users" ON user_points_log;
...
DROP POLICY IF EXISTS "Users can view their own assessments" ON productivity_assessments;
CREATE POLICY "Users can view their own assessments" ON productivity_assessments
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own assessments" ON productivity_assessments;
...
DROP POLICY IF EXISTS "System can insert challenges for users" ON user_challenges;
...
DROP TRIGGER IF EXISTS update_user_gamification_profiles_updated_at ON user_gamification_profiles;
...
BEGIN
  ...
  SELECT level, total_xp INTO old_level, current_total_xp
  ...
  new_level := get_level_from_xp(current_total_xp + final_points);
