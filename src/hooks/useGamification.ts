...
export function useGamification() {
      ...
      if (!existingProfile) {
        const { data: newProfile, error: insertError } = await supabase
          ...
          .insert({
            ...
            weekly_reset_at: new Date(Date.now() - Date.now() % (7 * 24 * 60 * 60 * 1000)).toISOString(),
  ...
  const awardPoints = useCallback(async (actionType: string, sourceId?: string, customPoints?: number) => {
    if (!user) {
  ...
  const checkAchievements = useCallback(async () => {
    ...
    try {
      ...
      const processedAchievements: AchievementProgress[] = achievementData.map(achievement => {
        ...
        return {
          ...
          progress_percentage: Math.min(100,
            ((userAchievement?.current_progress || 0) / achievement.requirement_value) * 100
          )
        };
      });
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      const [weeklyPoints, monthlyPoints, totalPoints, challengeData] = await Promise.all([
        supabase
      ...
      const stats: GamificationStats = {
        total_points_earned: totalPoints.data?.reduce((sum, entry) => sum + entry.points, 0) || 0,
        points_this_week: weeklyPoints.data?.reduce((sum, entry) => sum + entry.points, 0) || 0,
        points_this_month: monthlyPoints.data?.reduce((sum, entry) => sum + entry.points, 0) || 0,
        ...
        active_challenges: challengeData.data?.filter(c => c.status === 'active').length || 0,
        completed_challenges: challengeData.data?.filter(c => c.status === 'completed').length || 0
      };
