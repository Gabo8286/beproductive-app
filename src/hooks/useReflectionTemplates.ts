...
export function useReflectionTemplates(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (includeSystem) {
        query = query.or(
          `is_system.eq.true,created_by.eq.${user.data.user?.id},is_public.eq.true`,
        );
      } else {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({
        queryKey: ["reflections", data.workspace_id],
      });
    ...
    onError: (error: Error) => {
      toast.error(
        `Failed to create reflection from template: ${error.message}`,
      );
    },
  });
}
