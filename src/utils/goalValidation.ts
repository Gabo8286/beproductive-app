...
export function validateGoalInput(input: CreateGoalInput | UpdateGoalInput): string[] {
  const errors: string[] = [];
  ...
  // Date validation
  if ('start_date' in input && 'target_date' in input &&
      input.start_date && input.target_date) {
  ...
  // Value validation
  if ('current_value' in input && 'target_value' in input &&
      input.current_value !== undefined && input.target_value !== undefined) {
...
export function canDeleteGoal(goal: Goal, subGoals: Goal[]): { canDelete: boolean; reason?: string } {
...
export function sanitizeGoalInput<T extends CreateGoalInput | UpdateGoalInput>(input: T): T {
