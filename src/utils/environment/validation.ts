...
export function validateEnvironment(): ValidationResult {
  ...
  if (!supabaseKey) {
    ...
    warnings.push('VITE_SUPABASE_PUBLISHABLE_KEY does not appear to be a valid JWT token');
  } else {
  ...
  if (errors.length > 0 && import.meta.env.DEV) {
    errors.push('Environment variables are missing. Make sure you have a .env file in your project root with the required variables.');
  }
...
export function getEnvironmentInfo(): {
  ...
  return {
    ...
    variables: {
      ...
      supabaseUrlFormat: import.meta.env.VITE_SUPABASE_URL?.substring(0, 20) + '...',
      supabaseKeyFormat: import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY?.substring(0, 10) + '...',
    },
  };
}
