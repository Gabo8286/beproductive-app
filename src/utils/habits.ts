...
export function getNextDueDate(
  ...
  switch (habit.frequency) {
    ...
    case "weekly": {
      ...
      const weeksSinceCompletion = Math.floor(
        differenceInDays(today, lastCompletedDate) / 7,
      );
    ...
    case "monthly": {
      ...
      const monthsSinceCompletion = Math.floor(
        differenceInDays(today, lastCompletedDate) / 30,
      );
...
function calculateCustomNextDue(
...
export function calculateHabitScore(
  ...
  const recentEntries = entries.filter(
    (e) => parseISO(e.date) >= thirtyDaysAgo && e.status === "completed",
  );
...
export function validateEntryInput(input: {
  ...
  if (
    input.difficulty_felt !== undefined &&
    (input.difficulty_felt < 1 || input.difficulty_felt > 10)
  ) {
