import type { MoodLevel, Reflection, ReflectionInsight } from "@/types/reflections";
 ...
 */
export function generateReflectionSummary(reflection: Reflection, maxLength: number = 150): string {
 ...
 */
export function generateInsights(reflections: Reflection[]): ReflectionInsight[] {
  ...
  if (moodScores.length >= 3) {
    ...
    const overallMood = moodScores.reduce((a, b) => a + b, 0) / moodScores.length;
    
    if (recentMood > overallMood + 0.5) {
      insights.push({
        type: 'mood_trend',
        title: 'Improving Mood Trend',
        description: 'Your recent mood has been improving compared to your average. Keep up the positive momentum!',
        priority: 'medium',
      });
      ...
      insights.push({
        ...
        description: 'Your recent mood has been lower than usual. Consider reviewing what might be contributing to this.',
  ...
  if (energyLevels.length >= 5) {
    ...
    if (avgEnergy < 4) {
      insights.push({
        ...
        description: 'Your energy levels have been consistently low. Consider reviewing your sleep, nutrition, and exercise habits.',
  ...
  if (stressLevels.length >= 3) {
    ...
    if (highStressDays >= 2) {
      insights.push({
        ...
        description: 'You\'ve experienced high stress levels recently. Consider stress management techniques or seeking support.',
  ...
  if (!hasGratitude && reflections.length >= 3) {
    insights.push({
      ...
      description: 'Adding gratitude items to your reflections can improve mood and perspective. Give it a try!',
  ...
  if (!hasLearnings && reflections.length >= 3) {
    insights.push({
      ...
      description: 'Recording key learnings helps reinforce growth and development. Try adding learnings to your reflections.',
...
export function validateReflectionInput(input: {
  ...
  if (input.stress_level !== undefined && (input.stress_level < 1 || input.stress_level > 10)) {
    errors.push('Stress level must be between 1 and 10');
  }
  
  if (input.satisfaction_level !== undefined && (input.satisfaction_level < 1 || input.satisfaction_level > 10)) {
