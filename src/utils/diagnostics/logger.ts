...
class DiagnosticLogger {
  ...
  completeStep(step: string, metadata?: Record<string, any>): void {
    ...
    console.log(`[Diagnostics] ✅ ${step} - Completed (${duration}ms)`, metadata || '');
  }

  /**
   * Mark a diagnostic step as failed
   */
  failStep(step: string, error: string | Error, metadata?: Record<string, any>): void {
    const endTime = Date.now();
    const startTime = this.stepTimers.get(step);
    const duration = startTime ? endTime - startTime : undefined;
    ...
    console.error(`[Diagnostics] ❌ ${step} - Failed (${duration}ms):`, error, metadata || '');
  }
  ...
  getReport(): {
    ...
    const firstStep = this.steps.length > 0 ? this.steps[0].timestamp : Date.now();
    const lastStep = this.steps.length > 0 ? this.steps[this.steps.length - 1].timestamp : Date.now();
  ...
  printReport(): void {
    ...
    report.steps.forEach((step, index) => {
      ...
      console.log(`  ${index + 1}. ${icon} ${step.step}${duration}`, step.error || step.metadata || '');
    });
  ...
  hasCriticalFailures(): boolean {
    ...
    return this.steps.some(step =>
      criticalSteps.includes(step.step) && step.status === 'failed'
    );
  }
}
