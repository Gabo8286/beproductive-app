...
export default function Goals() {
  ...
  const hierarchicalGoals = useMemo(() => {
    ...
    return parentGoals.map((parent) => ({
      ...parent,
      children: childGoals.filter(
        (child) => child.parent_goal_id === parent.id,
      ),
    }));
      ...
      <div className="grid gap-4 md:grid-cols-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Badge variant="default">{goalStats.active}</Badge>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Badge variant="outline">{goalStats.archived}</Badge>
          </CardHeader>
        </Card>
      </div>
        ...
        <TabsContent value={viewMode} className="mt-6">
          {processedGoals.length === 0 ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {(showHierarchy ? hierarchicalGoals : processedGoals).map(
                (goal: any) => (
                  <div key={goal.id}>
                    <GoalCard goal={goal} />
                    {showHierarchy &&
