...
export default function Habits() {
  ...
  const totalStreakDays = activeHabits.reduce((sum, h) => sum + h.current_streak, 0);
  const todayCompletionRate = todayEntries?.length 
    ? (todayEntries.filter(e => e.status === 'completed').length / todayEntries.length) * 100 
    : 0;
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold">{todayCompletionRate.toFixed(0)}%</div>
