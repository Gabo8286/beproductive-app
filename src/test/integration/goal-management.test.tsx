...
describe("Goal Management Integration Tests", () => {
  ...
  describe("Goal Deletion Workflow", () => {
    it("should complete goal deletion with confirmation", async () => {
      ...
      expect(
        screen.getByText("Are you sure you want to delete this goal?"),
      ).toBeInTheDocument();
      expect(
  ...
  describe("Goal Progress Update Workflow", () => {
    it("should update goal progress through various methods", async () => {
      ...
      vi.mocked(
        require("@/hooks/useGoals").useUpdateGoalProgress,
      ).mockReturnValue({
      ...
      expect(screen.getByTestId("progress-value")).toHaveTextContent(
        "Progress: 25%",
      );
  ...
  describe("Error Recovery and Edge Cases", () => {
      ...
      const NetworkTestComponent = () => {
        ...
        return (
          <div>
            ...
            {status === "loading" && (
              <div data-testid="loading">Updating...</div>
            )}
            {status === "error" && (
            ...
            {status === "success" && (
              <div data-testid="success">Updated successfully!</div>
            )}
    ...
    it("should handle concurrent operations safely", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("completed-ops")).toHaveTextContent(
          "Completed: 3",
        );
      });
