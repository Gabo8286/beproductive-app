...
export class HabitFactory extends BaseFactory<Habit> {
  protected create(overrides: Partial<Habit> = {}): Habit {
    ...
    const baseHabit: Habit = {
      ...
      reminderTime: faker.datatype.boolean()
        ? faker.date.recent().toTimeString().slice(0, 5)
        : undefined,
      isActive: faker.datatype.boolean(),
      streak,
      longestStreak,
      completions: this.createCompletions(
  ...
  public buildWithStreak(streak: number, overrides?: Partial<Habit>): Habit {
    return this.build({
      ...
      longestStreak: Math.max(
        streak,
        faker.number.int({ min: streak, max: streak + 20 }),
      ),
      ...overrides,
    });
  }
...
export class ProjectFactory extends BaseFactory<Project> {
  protected create(overrides: Partial<Project> = {}): Project {
    const baseProject: Project = {
      ...
      goals: Array.from({ length: faker.number.int({ min: 2, max: 10 }) }, () =>
        faker.string.uuid(),
      ),
...
export class DataFactoryManager {
  ...
  public createActiveUserScenario(): {
    ...
    const habits = [
      ...
      this.habitFactory.buildWithStreak(0, {
        userId: user.id,
        isActive: false,
      }),
    ];
