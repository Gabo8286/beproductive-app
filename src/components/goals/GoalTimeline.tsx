...
export function GoalTimeline({ goal, milestones }: GoalTimelineProps) {
  ...
  const getEventColor = (type: string, completed?: boolean) => {
    switch (type) {
      ...
      case "milestone": return completed ? "bg-green-500" : "bg-yellow-500";
      case "milestone-completed": return "bg-green-500";
      case "target": return "bg-orange-500";
        ...
        <div className="space-y-4">
          {timelineEvents.map((event, index) => (
            <div key={index} className="flex items-start space-x-4">
              <div className={`w-2 h-2 rounded-full ${getEventColor(event.type, event.completed)} mt-2 flex-shrink-0`} />
