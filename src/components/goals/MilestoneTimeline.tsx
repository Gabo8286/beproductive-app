...
export function MilestoneTimeline({ milestones, goal }: MilestoneTimelineProps) {
  ...
  const getStatusColor = (milestone: GoalMilestone) => {
    ...
    if (milestone.target_date && isPast(new Date(milestone.target_date))) return "border-red-500";
    if (milestone.target_date && isFuture(new Date(milestone.target_date))) return "border-blue-500";
    return "border-muted";
  };
          ...
          <div className="relative space-y-6 before:absolute before:left-4 before:top-4 before:bottom-4 before:w-0.5 before:bg-border">
            {sortedMilestones.map((milestone, index) => (
              <div key={milestone.id} className="relative flex items-start gap-4 pl-12">
                <div className={`absolute left-0 top-1 p-1.5 rounded-full border-2 bg-background ${getStatusColor(milestone)}`}>
