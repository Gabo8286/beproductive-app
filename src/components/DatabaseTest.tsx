      ...
      <CardContent className="space-y-4">
        <div className="grid gap-2">
          <div className="flex justify-between">
            ...
            <span className={results.profile === 'PASSED' ? 'text-green-600' : 'text-red-600'}>
              {results.profile || 'Testing...'}
            </span>
          </div>
          <div className="flex justify-between">
            <span>Workspaces Table:</span>
            <span className={results.workspaces === 'PASSED' ? 'text-green-600' : 'text-red-600'}>
              {results.workspaces || 'Testing...'}
            </span>
          </div>
          <div className="flex justify-between">
            <span>Tasks Table:</span>
            <span className={results.tasks === 'PASSED' ? 'text-green-600' : 'text-red-600'}>
        ...
        {profile && (
          <div className="mt-4 p-3 bg-muted rounded">
            <p className="text-sm">
              <strong>User:</strong> {profile.full_name || 'No name'} ({profile.role})
            </p>
            <p className="text-sm">
