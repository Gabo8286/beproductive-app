...
export const IntelligentTimeTracker: React.FC<IntelligentTimeTrackerProps> = ({
  ...
  const [selectedCategory, setSelectedCategory] =
    useState<TimeEntry["category"]>("deep_work");
  ...
  const [estimationMode, setEstimationMode] = useState<"simple" | "detailed">(
    "simple",
  );
  const [taskToEstimate, setTaskToEstimate] = useState("");
  const [estimationFactors, setEstimationFactors] = useState<
    EstimationFactor[]
  >([]);
  ...
  useEffect(() => {
    ...
    if (isTracking && currentEntry) {
      interval = setInterval(() => {
        setElapsedTime(
          Date.now() - new Date(currentEntry.startTime!).getTime(),
        );
      }, 1000);
    }
    ...
    const mockEstimates: TimeEstimate[] = [
      {
        ...
        factors: [
          {
            ...
            impact: "increase",
            weight: 0.3,
            description: "High complexity backend integration",
          },
          {
            name: "Similar Experience",
            impact: "decrease",
            weight: 0.2,
            description: "Previous auth implementations",
          },
          {
            name: "Current Energy",
            impact: "neutral",
            weight: 0.1,
            description: "Moderate energy level",
          },
        ],
        ...
        updated_at: new Date(),
        ai_reasoning:
          "Based on similar tasks and current productivity patterns, estimated 2 hours with high confidence.",
      },
    ];
  ...
  const generateEstimate = async () => {
    ...
    const estimatedDuration =
      baseEstimate * complexityFactor * experienceFactor;
  ...
  const getCurrentProductivityTip = (): string => {
    ...
    const pattern = patterns.find((p) =>
      p.time_of_day.includes(currentHour.toString()),
    );
    if (pattern) {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              AI-powered time tracking with smart estimations and productivity
              insights
            </p>
          </div>
        </div>
        ...
        <TabsContent value="tracker" className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{getCurrentProductivityTip()}</CardDescription>
            </CardHeader>
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  <Select
                    ...
                    onValueChange={(value) =>
                      setSelectedCategory(value as TimeEntry["category"])
                    }
                  >
                      ...
                      <div className="flex items-center space-x-1 text-sm">
                        <div className="flex space-x-1">
                          <span className="text-green-600">
                            P: {entry.productivity_score}/10
                          </span>
                          <span className="text-blue-600">
                            F: {entry.focus_rating}/10
                          </span>
                        </div>
                      </div>
                    </div>
        ...
        <TabsContent value="estimates" className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Get intelligent time estimates based on task analysis and
                historical data
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setEstimationMode(value as "simple" | "detailed")
                }
              >
                <SelectTrigger className="w-48">
                  <SelectValue />
                ...
                {estimates.map((estimate) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        <Badge variant="outline">
                          {Math.round(estimate.confidence_level * 100)}%
                          confidence
                        </Badge>
                    ...
                    <div className="flex items-center space-x-4 text-sm text-muted-foreground">
                      <span>
                        Historical Accuracy:{" "}
                        {Math.round(estimate.historical_accuracy * 100)}%
                      </span>
                      <span>
                        Created: {estimate.created_at.toLocaleString()}
                      </span>
                    </div>
                  </motion.div>
                ))}
            ...
            <>
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {formatDuration(analytics.total_tracked)}
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {Math.round(analytics.estimation_accuracy * 100)}%
                    </div>
                  </CardContent>
                </Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {analytics.category_breakdown.map((category) => {
                      const percentage =
                        (category.duration / analytics.total_tracked) * 100;
                      return (
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {analytics.improvement_suggestions.map(
                      (suggestion, index) => (
                        <motion.div
                          key={index}
