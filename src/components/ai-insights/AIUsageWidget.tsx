...
export const AIUsageWidget: React.FC = () => {
  ...
  const getUsagePatterns = () => {
    if (!usageStats?.daily_breakdown || usageStats.daily_breakdown.length === 0)
      return null;

    const recent7Days = usageStats.daily_breakdown.slice(-7);
    const avgDailyCost =
      recent7Days.reduce((sum, day) => sum + day.cost, 0) / recent7Days.length;
    const avgDailyRequests =
      recent7Days.reduce((sum, day) => sum + day.requests, 0) /
      recent7Days.length;
  ...
  const projectedExceedsLimit =
    projections && projections.projected_month_end > 10; // $10 default user limit
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm mb-4">
              <div>
                ...
                <p className="font-bold">
                  {formatCurrency(projections.current_month_cost)}
                </p>
              </div>
              <div>
                <p className="text-gray-600">Projected total:</p>
                <p
                  className={`font-bold ${projectedExceedsLimit ? "text-amber-600" : "text-green-600"}`}
                >
              ...
              <ResponsiveContainer width="100%" height={80}>
                <LineChart
                  data={[
                    {
                      day: "Today",
                      actual: projections.current_month_cost,
                      projected: projections.current_month_cost,
                    },
                    {
                      day: "Mid-month",
                      actual: null,
                      projected:
                        projections.current_month_cost +
                        projections.daily_average *
                          Math.floor(projections.days_remaining / 2),
                    },
                    {
                      day: "Month-end",
                      actual: null,
                      projected: projections.projected_month_end,
                    },
                  ]}
                >
            ...
            <div className="mt-2">
              <div className="flex justify-between text-xs text-gray-500 mb-1">
                ...
                <span>
                  {((projections.projected_month_end / 10) * 100).toFixed(0)}%
                </span>
              </div>
              <Progress
                value={Math.min(
                  (projections.projected_month_end / 10) * 100,
                  100,
                )}
                className="h-2"
              />
              {projectedExceedsLimit && (
                <p className="text-xs text-amber-600 mt-1">
                  ⚠ Projected to exceed $10 budget by{" "}
                  {formatCurrency(projections.projected_month_end - 10)}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-3">
            {usageLoading ? (
                ...
                return providers
                  ...
                  .map(([provider, stats]) => {
                    ...
                    const requestPercentage =
                      totalRequests > 0
                        ? (stats.requests / totalRequests) * 100
                        : 0;
                    const costPerRequest =
                      stats.requests > 0 ? stats.cost / stats.requests : 0;
                    const costPerToken =
                      stats.tokens > 0 ? stats.cost / stats.tokens : 0;
                        ...
                        <div className="grid grid-cols-3 gap-3 text-xs">
                          <div className="text-center">
                            ...
                            <p className="text-gray-500">
                              {requestPercentage.toFixed(1)}% requests
                            </p>
                          </div>
                          <div className="text-center">
                            <p className="font-medium">
                            ...
                            <p className="text-gray-500">
                              {formatCurrency(costPerToken * 1000)}/1K tokens
                            </p>
                          </div>
                          <div className="text-center">
              ...
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-xs">
                  Generate some insights to see your usage here!
                </p>
              </div>
            )}
          </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <p className="text-gray-600">Daily Average:</p>
                <p className="font-bold">
                  {formatCurrency(patterns.avgDailyCost)}
                </p>
                <p className="text-xs text-gray-500">
                  {Math.round(patterns.avgDailyRequests)} requests/day
                </p>
              </div>
              <div>
                <p className="text-gray-600">Efficiency Score:</p>
                <p className="font-bold">
                  {Math.round(patterns.efficiency)} tokens/$
                </p>
                <p className="text-xs text-gray-500">
                  {patterns.efficiency > 1000
                    ? "Excellent"
                    : patterns.efficiency > 500
                      ? "Good"
              ...
              <div>
                ...
                <p className="font-bold">
                  {new Date(patterns.peakDay.date).toLocaleDateString("en", {
                    weekday: "short",
                  })}
                </p>
                <p className="text-xs text-gray-500">
                  {formatCurrency(patterns.peakDay.cost)}
                </p>
              </div>
              <div>
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-amber-700 mt-1">
                  You're approaching higher usage levels. Consider reviewing
                  your AI insight generation frequency or focusing on specific
                  insight types to optimize costs.
                </p>
              </div>
            </div>
          </div>
        ...
        {usageStats &&
          ...
          usageStats.daily_breakdown.length > 0 && (
            <div>
              <h4 className="font-medium text-gray-900 mb-3 flex items-center gap-2">
