...
interface AIRecommendationCardProps {
  ...
  onUpdateStatus: (id: string, status: 'pending' | 'in_progress' | 'completed' | 'dismissed') => void;
}

export function AIRecommendationCard({ recommendation, onUpdateStatus }: AIRecommendationCardProps) {
  const effortColors = {
          ...
          <div className="flex items-start gap-3 flex-1">
            ...
            <div className="flex-1 space-y-1">
              ...
              <CardDescription className="flex items-center gap-2">
                <Badge variant="secondary">{statusLabels[recommendation.status]}</Badge>
      ...
      <CardContent className="space-y-4">
        ...
        {recommendation.expected_impact && (
          <div className="p-3 bg-accent rounded-lg">
            ...
            <p className="text-sm text-muted-foreground mt-1">{recommendation.expected_impact}</p>
          </div>
        )}
