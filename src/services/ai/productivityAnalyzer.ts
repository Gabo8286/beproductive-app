...
export class ProductivityAnalyzer {
  ...
  async analyzeProductivityPattern(data: ProductivityData, userId: string): Promise<AnalysisResult | null> {
...
Provide insights about this user's productivity patterns and suggest optimizations.`,
    ...
    return {
      ...
      content: aiResponse.content || `Your productivity peaks at ${peakHour ? `${peakHour}:00` : 'various times'} and you're most productive on ${peakDay ? weekdays[parseInt(peakDay)] : 'multiple days'}. Consider scheduling important work during these peak times.`,
  ...
  async analyzeTimeOptimization(data: ProductivityData, userId: string): Promise<AnalysisResult | null> {
    if (data.tasks.length === 0) return null;

    const tasksWithTime = data.tasks.filter(t => t.estimated_duration && t.actual_duration);
    const timeAccuracy = tasksWithTime.length > 0
      ? tasksWithTime.reduce((acc, task) => {
          const accuracy = 1 - Math.abs(task.actual_duration - task.estimated_duration) / task.estimated_duration;
          return acc + Math.max(0, accuracy);
        }, 0) / tasksWithTime.length
      : 0;

    const avgTaskDuration = data.tasks
      .filter(t => t.actual_duration)
      .reduce((sum, t) => sum + t.actual_duration, 0) / data.tasks.filter(t => t.actual_duration).length || 0;
...
Suggest time optimization strategies based on this data.`,
    ...
    return {
      ...
      content: aiResponse.content || `Your time estimation accuracy is ${(timeAccuracy * 100).toFixed(1)}%. ${timeAccuracy > 0.8 ? 'Great job with time estimation!' : 'Consider improving time estimation skills.'}`,
      summary: `${(timeAccuracy * 100).toFixed(0)}% estimation accuracy`,
      confidence_score: tasksWithTime.length > 5 ? 0.85 : 0.6,
      data_sources: ['tasks'],
      recommendations: timeAccuracy < 0.7 ? [{
        title: 'Improve Time Estimation',
        description: 'Better time estimates lead to better planning and reduced stress.',
        implementation_steps: [
        ...
        description: 'Your time estimation is good! Try batching similar tasks.',
        implementation_steps: [
  ...
  async analyzeProjectHealth(data: ProductivityData, userId: string): Promise<AnalysisResult | null> {
    if (data.projects.length === 0) return null;

    const activeProjects = data.projects.filter(p => p.status === 'active');
    const overdueProjects = data.projects.filter(p =>
      p.target_date && new Date(p.target_date) < new Date() && p.status !== 'completed'
    );
    const avgProgress = data.projects.reduce((sum, p) => sum + (p.progress_percentage || 0), 0) / data.projects.length;
...
Assess project health and suggest improvements.`,
    ...
    return {
      type: 'project_health',
      title: isHealthy ? 'Healthy Project Portfolio' : 'Project Portfolio Needs Attention',
      content: aiResponse.content || `You have ${data.projects.length} projects with ${overdueProjects.length} overdue. Average progress is ${avgProgress.toFixed(1)}%. ${isHealthy ? 'Your projects are on track!' : 'Some projects need attention.'}`,
      summary: `${overdueProjects.length} overdue of ${data.projects.length} total`,
      confidence_score: 0.9,
      data_sources: ['projects'],
      recommendations: overdueProjects.length > 0 ? [{
        title: 'Address Overdue Projects',
        description: 'Overdue projects can cascade delays. Address them immediately.',
        implementation_steps: [
        ...
        description: 'Your projects are healthy! Keep the momentum going.',
        implementation_steps: [
  ...
  async analyzeBurnoutRisk(data: ProductivityData, userId: string): Promise<AnalysisResult | null> {
    if (data.reflections.length === 0 && data.tasks.length === 0) return null;

    // Calculate stress indicators
    const recentReflections = data.reflections.slice(0, 7);
    const moodValues = { 'amazing': 5, 'great': 4, 'good': 3, 'neutral': 2, 'bad': 1, 'terrible': 0 };
    const avgMood = recentReflections.length > 0
      ? recentReflections.reduce((sum, r) => sum + (moodValues[r.mood as keyof typeof moodValues] || 2), 0) / recentReflections.length
      : 3;
    ...
    const workload = data.tasks.filter(t =>
...
Evaluate burnout risk and suggest prevention strategies.`,
    ...
    const riskLevel = avgMood < 2.5 || overdueTasks > 10 || workload > 20 ? 'high' :
                     avgMood < 3.5 || overdueTasks > 5 || workload > 15 ? 'medium' : 'low';

    return {
      type: 'burnout_risk',
      title: `Burnout Risk: ${riskLevel.toUpperCase()}`,
      content: aiResponse.content || `Based on your recent mood (${avgMood.toFixed(1)}/5), ${overdueTasks} overdue tasks, and ${workload} new tasks this week, your burnout risk is ${riskLevel}.`,
      ...
      recommendations: riskLevel === 'high' ? [{
        ...
        description: 'High burnout risk detected. Take immediate action to reduce stress.',
        implementation_steps: [
        ...
        description: 'Your burnout risk is manageable. Keep healthy habits.',
        implementation_steps: [
  ...
  async analyzeAchievementOpportunity(data: ProductivityData, userId: string): Promise<AnalysisResult | null> {
    if (data.goals.length === 0 && data.habits.length === 0) return null;

    const nearCompleteGoals = data.goals.filter(g =>
      g.status === 'in_progress' && (g.progress || 0) > 80
    );

    const strongHabits = data.habits.filter(h => (h.current_streak || 0) >= 21);
    const emergingHabits = data.habits.filter(h =>
      (h.current_streak || 0) >= 7 && (h.current_streak || 0) < 21
    );
...
Suggest achievable wins and momentum-building opportunities.`,
    ...
    const hasOpportunities = nearCompleteGoals.length > 0 || emergingHabits.length > 0;

    return {
      type: 'achievement_opportunity',
      title: hasOpportunities ? 'Achievement Opportunities Ahead!' : 'Building Achievement Momentum',
      content: aiResponse.content || `You have ${nearCompleteGoals.length} goals near completion and ${emergingHabits.length} habits building momentum. ${hasOpportunities ? 'Focus on these quick wins!' : 'Keep building consistency to create future opportunities.'}`,
      ...
      recommendations: hasOpportunities ? [{
        ...
        description: 'You have achievements within reach. Focus on completing them.',
        implementation_steps: [
        ...
        description: 'Focus on building consistent habits that will create future wins.',
        implementation_steps: [
  ...
  async generateInsights(userId: string, types?: AIInsightType[]): Promise<AnalysisResult[]> {
    ...
    const analyzers: Record<AIInsightType, () => Promise<AnalysisResult | null>> = {
      ...
      reflection_sentiment: async () => this.analyzeReflectionSentiment(data.reflections),
      productivity_pattern: async () => this.analyzeProductivityPattern(data, userId),
      ...
      achievement_opportunity: async () => this.analyzeAchievementOpportunity(data, userId),
    };
